...
from tests.unit.test_utils.users.user_repository_fixture import UserRepositoryFixture
...
def when_register(user_service: UserServiceInterface, username: str, password: str) -> User:
    return user_service.register(username, password)


def then_user_is_registered(test_case: CustomTestCase, user: User, username: str, password: str) -> None:
...
def then_error_message_is_username_already_exists(test_case: CustomTestCase, exception: Exception) -> None:
